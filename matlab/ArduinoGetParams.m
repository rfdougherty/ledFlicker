...
for ii = 1:size(Conditions, 2)
    Conditions{ii}.StimRept = 3;    
end

%% sequence prameteres

Envelope.StimDur = 4;   % (sec):stimulus duration, which should be integral multiple of TRs for optseq2.
...
Sequence.TR       = 2;      %(sec):TR
Sequence.PSDwin   = 12;     %(sec):PSD window should ve long enouph catpure response. Post-stimulus delay (PSD = 0 = Stimulus onset)
